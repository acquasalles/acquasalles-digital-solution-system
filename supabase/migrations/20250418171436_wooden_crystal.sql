/*
  # Generate data for updated collection points
  
  1. Changes
    - Generate data from March 19th, 2025 until now
    - Use new measurement structure
    - Include all measurement types in their new locations
...
BEGIN
  ...
  IF _area_id IS NULL OR _ponto_a1_id IS NULL OR _ponto_a2_id IS NULL THEN
    RAISE EXCEPTION 'Required IDs not found. Area: %, Ponto A1: %, Ponto A2: %', _area_id, _ponto_a1_id, _ponto_a2_id;
  END IF;
